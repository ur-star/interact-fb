...
async function getUserData() {
  try {
    ...
    if (error instanceof FacebookAuthError) {
      ...
      console.error(
        "Permission Error: Missing required access. Please grant permissions.",
        error.message
      );
      // Guide the user on which permissions are needed
    } else if (error instanceof FacebookAPIError) {
      console.error(
        "Facebook API Error: Something went wrong with the request.",
        error.message
      );
